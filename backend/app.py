...
def initialize_firebase():
    if not FIREBASE_SECRET_FULL_PATH:
        raise RuntimeError("CRITICAL: FIREBASE_SECRET_FULL_PATH env variable not set")
    try:
...
# Firestore client (always available after successful initialization)
db = firestore.client()
...
Knowledge sources: Saheeh International, Tafsir Ibn Kathir, Tafsir al-Qurtubi, Tafsir al-Jalalayn, Tafsir al-Tabari.
JSON schema:
{{
...
"""
    ...
    if level == "casual":
        system_instruction += "Keep explanations short, simple, practical.\n"
    elif level == "beginner":
        system_instruction += "Explain concepts in simple English with definitions of Arabic terms.\n"
    elif level == "intermediate":
        system_instruction += "Include brief linguistic notes and moderate depth.\n"
    elif level == "advanced":
        system_instruction += "Provide full linguistic and theological nuance with scholar comparisons.\n"

    # Focus-specific
    if focus == "practical":
        system_instruction += "Emphasize lessons and applications for daily life.\n"
    elif focus == "linguistic":
        system_instruction += "Emphasize Arabic grammar, roots, and rhetorical devices.\n"
    elif focus == "comparative":
        system_instruction += "Compare views of different tafsīr scholars.\n"
    elif focus == "thematic":
        system_instruction += "Connect verses across the Qur'an on the theme.\n"
    ...
    if verbosity == "short":
        ...
        system_instruction += "Provide in-depth analysis with examples.\n"
...
def set_profile():
    ...
    try:
        db.collection("users").document(uid).set(
            {"level": level, "focus": focus, "verbosity": verbosity}, merge=True
        )
...
def tafsir_handler():
    ...
    try:
        ...
        if "candidates" in raw and raw["candidates"]:
            try:
                generated_text = raw["candidates"][0]["content"]["parts"][0]["text"]
